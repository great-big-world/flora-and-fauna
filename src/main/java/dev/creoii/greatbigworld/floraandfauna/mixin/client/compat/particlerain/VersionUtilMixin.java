...
public class VersionUtilMixin {
    ...
    private static Biome.Precipitation gbw$modifyCanSetSnowForWinter(Biome instance, BlockPos pos, int seaLevel, Operation<Biome.Precipitation> original, @Local(argsOnly = true, name = "arg0") World level) {
        if (level.isClient() && (FloraAndFaunaClient.getCurrentSeason() == Season.WINTER && !level.getBiome(pos).isIn(FloraAndFaunaTags.NOT_AFFECTED_BY_WINTER) && Blocks.SNOW.getDefaultState().canPlaceAt(level, pos) && level.isInHeightLimit(pos.getY()) && level.getLightLevel(LightType.BLOCK, pos) < 10))
            ...
            if (level instanceof ServerWorld serverWorld) {
                ...
                return (seasonManager.getCurrentSeason() == Season.WINTER && !level.getBiome(pos).isIn(FloraAndFaunaTags.NOT_AFFECTED_BY_WINTER) && Blocks.SNOW.getDefaultState().canPlaceAt(level, pos) && level.isInHeightLimit(pos.getY()) && level.getLightLevel(LightType.BLOCK, pos) < 10) ? Biome.Precipitation.SNOW : original.call(instance, pos, seaLevel);
